...
- `UNC_FILL`填充确认支持新增、编辑和删除手动清单项，例如手动添加`插座 / 个 / 2`。
- 明确拆分 `SUM-STAT/求和统计` 与 `BOQ-TABLE/清单表格` 模块；日志和错误均显示模块ID、阶段及耗时。
- 填充配置改为单次不可变快照，软管默认米数和未匹配管材默认选择可配置。
...
> 若命令可用但UNCAD页签未显示，执行 `UNC_RIBBON`；若 `UNC_ABOUT` 提示未知命令，请重新运行v1.5.2安装器。安装器会解除下载文件阻止并恢复命令触发加载。当前用户安装仅对执行安装的Windows账号生效，共用电脑应选择所有用户安装。
...
- 刚性线管和软管均按直径严格匹配固定清单，不用其他直径模板代替。固定清单找不到规格时，“填充确认”自动打开清单选择页并标出缺失型号；缺失型号默认不勾选，用户可逐项选择本次是否仍要生成。
- 选择设备/回路后会打开“填充确认”：自动匹配到的电缆、桥架、线管、软管、断路器、插座等默认全部勾选。
- 可取消不需要的项目（例如插座盘不生成插座），也可直接修改基础信息、电缆型号/长度、软管直径及每行名称、特征、单位、数量和项目编码。修改软管直径时，预览中的软管名称、特征、单位和项目编码会立即按固定清单重新匹配，当前数量保持不变。
