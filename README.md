...
| `UNC_F` | —（新增） | 按机台/设备回路生成单个清单和图框属性，完成后自动更新 XLSX |
| `UNC_UPDATE` | —（新增） | 读取已填身份并按图框边界批量更新，完成后自动批量更新 XLSX |
| `UNC_ABOUT` | —（新增） | 查看版本、构建时间、授权状态、所有权和联系方式 |
| `UNC_RIBBON` | —（新增） | 显示Ribbon并重新注册隐藏的UNCAD页签 |
| `UNC_CONDUIT` | —（新增） | 生成紫色偏移线管及固定 `2000mm` 占位标注 |
...
## UNC_F / UNC_UPDATE 数据源

`UNC_F` 和 `UNC_UPDATE` 将频繁更新的机台数据与固定 BOQ 清单分开管理：
...
- 已填充后修改了桥架或多处电缆长度，可执行 `UNC_UPDATE`：命令从图框/设备块自动读取机台ID和设备名称并定位 Excel 回路；一次选中多个 `frame_20260812` 时按真实外框批量更新。
- 更新身份兼容新版 `MACHINEID-POWER + DEVICENAME` 和旧版 `MACHINEID-DEVICE`；无法唯一定位同机台多回路时会中止并提示使用普通填充。

## 自动 Excel 记录

- 不再提供 `UNC_SUBMIT` 命令；每次 `UNC_F` 或 `UNC_UPDATE` 成功写图后自动更新配置目录下的 `UNCAD_Submissions.xlsx`。
- 输出目录在 `UNC_SET → Excel 数据 → 自动记录文件夹` 中设置；为空时默认使用当前用户“文档”目录。
...
| `UNC_SUBMIT_FOLDER` | `UNC_F` / `UNC_UPDATE` 自动记录文件夹（保留旧键名兼容已有设置） | 空（默认“文档”目录） |
