...
- `提交记录`改为不可删除的历史日志：每次 `U1F` 或 `U1U` 都追加一行，之前的提交全部保留。
- `清单明细`每个“机台ID + 设备名称”只占一行；多项材料在对应字段单元格内逐行排列，再次更新时整行替换为最新版本。
- 两个工作表都按机台ID稳定分组，相同ID的设备连续排列；`提交记录`内部仍保留每次提交历史。
- `U1U`不更新插座：现有 CAD 插座原样保留，用户删掉插座后不会根据机台 Excel 再次生成。
- 命令行分别显示“提交历史新增”和“最新数据替换”，避免把历史追加误报为覆盖。

## v1.9.5 重点更新

- 新版命令统一为 `U1` 前缀：`U1L`、`U1R`、`U1Q1/2/4`、`U1F`、`U1U`、`U1C`、`U1A`、`U1S`。
- 仅保留六个传统键盘命令：`UNL`、`UNR`、`UNQ1/2/4`、`UNADD`；移除其余 `UNC_*`、`OPUN*` 和 `UNADDX` 注册。
- `U1F` 生成和 `U1U` 单框/批量更新继续自动维护 `UNCAD_Submissions.xlsx`。
...
- 不再提供 `UNC_SUBMIT` 命令；每次 `U1F` 或 `U1U` 成功写图后自动更新配置目录下的 `UNCAD_Submissions.xlsx`。
- 输出目录在 `U1S → Excel 数据 → 自动记录文件夹` 中设置；为空时默认使用当前用户“文档”目录。
- `提交记录`工作表是完整历史：每次执行都新增一行，输出机台ID、设备名称、盘柜类型、设备原电缆型号、清单替代电缆型号及米数、软管直径及米数、桥架/线管信息及米数、FR、配电详情、上下游轴位和本次提交时间，旧行不删除。
- `清单明细`工作表是每个设备一行的最新状态：序号、材料名称、特征描述、单位、数量和项目编码分别在单元格内换行并保持一一对应；再次更新时替换该设备整行。
- 自动记录以命令刚写入的图纸清单表为准；图框属性仅在表格无对应行时后备使用。
- 两个工作表都按机台ID分组，相同ID的所有设备连续排列；不合并单元格，仍可逐行筛选和复制。
- 自动更新结束后，命令行显示提交历史新增数、最新数据替换数、当前材料明细数和完整文件路径。
