...
namespace UNCAD.Features.Fill
{
    ...
    internal static class BatchFillUpdateCoordinator
    {
        ...
        public static void Execute(CadContext ctx, IReadOnlyList<FrameRegionGroup> regions)
        {
            ...
            {
                ctx.Write("\n[UNC_FILL_UPDATE] 读取机台 Excel 失败: " + ex.Message);
                Log.Error("UNC_FILL_UPDATE batch read machine excel failed", ex);
                return;
            }
            if (workbook.MachineIds.Count == 0 || workbook.ListItems.Count == 0)
            {
                ctx.Write("\n[UNC_FILL_UPDATE] 机台 Excel 或内嵌固定清单没有可用数据。");
                return;
            }
            ...
            if (errors.Count > 0)
            {
                ...
                MessageBox.Show(Owner(), message, "UNC_FILL_UPDATE 批量预检",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                foreach (string error in errors) ctx.Write("\n[UNC_FILL_UPDATE] " + error);
                return;
            }
            ...
            if (MessageBox.Show(Owner(), summary.ToString(), "UNC_FILL_UPDATE 批量确认",
            ...
            {
                Log.Error("UNC_FILL_UPDATE batch write failed", ex);
                ctx.Write("\n[UNC_FILL_UPDATE] 批量写入失败，整批已回滚: " + ex.Message);
                MessageBox.Show(Owner(), "批量写入失败，所有图框修改均已回滚。\r\n\r\n" + ex.Message,
                    "UNC_FILL_UPDATE", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            SelectionService.ClearPickFirst(ctx);
            ctx.Write("\n[UNC_FILL_UPDATE] 批量完成：图框 " + plans.Count
